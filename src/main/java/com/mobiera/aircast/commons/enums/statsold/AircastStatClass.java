package com.mobiera.aircast.commons.enums.statsold;

import com.mobiera.ms.commons.stats.api.StatClass;

public enum AircastStatClass implements StatClass {

	SMPP_ACCOUNT(0,SmppAccountStat.class.getSimpleName(), true),
	CAMPAIGN(1,CampaignStat.class.getSimpleName(), true),
	CAMPAIGN_PARAMS(2,CampaignStat.class.getSimpleName(), false),
	ENDPOINT(3,EndpointStat.class.getSimpleName(), true),
	ACTION_TRIGGER(4,ActionTriggerStat.class.getSimpleName(), false),
	CAMPAIGN_SCHEDULE(5,CampaignScheduleStat.class.getSimpleName(), false),
	STK_ACTION(6,StkActionStat.class.getSimpleName(), false),
	STK_ACTION_DATA(7,StkActionDataStat.class.getSimpleName(), false),
	STK_ACTION_ITEM(8,StkActionItemStat.class.getSimpleName(), false);
	
	
	
	private boolean showByDefault;
	
	
	private AircastStatClass(Integer index, String className, boolean showByDefault){
		this.index = index;
		this.statClassName = className;
	}

	private String statClassName;
	
	public String getStatClassName() {
		return statClassName;
	}
	
	
	private Integer index;

	public Integer getIndex(){
		return this.index;
	}

	public static AircastStatClass getEnum(Integer index){
		if (index == null)
	return null;
		
		switch(index){
			case 0: return SMPP_ACCOUNT;
			case 1: return CAMPAIGN;
			case 2: return CAMPAIGN_PARAMS;
			case 3: return ENDPOINT;
			case 4: return ACTION_TRIGGER;
			case 5: return CAMPAIGN_SCHEDULE;
			case 6: return STK_ACTION;
			case 7: return STK_ACTION_DATA;
			case 8: return STK_ACTION_ITEM;
			default: return null;
		}
	}

	public boolean isShowByDefault() {
		return showByDefault;
	}

	public void setShowByDefault(boolean showByDefault) {
		this.showByDefault = showByDefault;
	}

	public void setStatClassName(String statClassName) {
		this.statClassName = statClassName;
	}

}